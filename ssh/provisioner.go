...
// shared.ConnectionInfo is decoded from the ConnInfo of the resource. These are the
...
func decodeConnInfo(v cty.Value) (*shared.ConnectionInfo, error) {
	connInfo := &shared.ConnectionInfo{}
	if v.IsNull() {
		return connInfo, nil
	}
...
// parseshared.ConnectionInfo is used to convert the raw configuration into the
// *shared.ConnectionInfo struct.
func parseConnectionInfo(ci shared.ConnectionInfo) (*shared.ConnectionInfo, error) {

	// Start with the provided connection info, we'll modify it as appropriate
	connInfo := ci
...
// prepareSSHConfig is used to turn the *shared.ConnectionInfo provided into a
// usable *SSHConfig for client initialization.
func prepareSSHConfig(connInfo *shared.ConnectionInfo) (*sshConfig, error) {
...
func connectToAgent(connInfo *shared.ConnectionInfo) (*sshAgent, error) {
	if !connInfo.Agent {
		// No agent configured
		return nil, nil
	}
